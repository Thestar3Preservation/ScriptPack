...
case $1 in
	...
	-v | --version)
		cat <<-EOF
			...
			2.9 논리부정이 비트부정되던 문제 해결. 정상적으로 부정연산이 가능해짐.
...
#계산식만 주어진다면
	...
	#'진리표'란 글자가 위치한 공간을 계산
	excelCol=$(($(echo -n $value_name | wc -m) * 8 + 4))
	maxCol=$(tput cols) #만약 노틸러스 스크립트에서 구동한다면, tput cols는 아무런 값도 출력하지 않는다.
	((maxCol <= excelCol)) && temp=$maxCol || temp=$excelCol
	selCol=$((temp / 2 - 4))

	#각각의 경우를 계산해서 다듬고 진리표를 출력
	echo -e "$COL_CYAN대수식 :$COL_RESET $expression$COL_CYAN\n\n$(perl -e "print ' ' x $selCol")=진리표=\n$(sed 's/\(.\)/\1\t/g' <<<"$value_name")출력$COL_RESET"
