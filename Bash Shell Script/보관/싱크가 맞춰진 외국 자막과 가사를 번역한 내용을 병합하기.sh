...
if eval "zenity --list --title '자막과 번역된 가사의 짝맞춤 확인' --text '아래 목록에서 올바른 짝맞춤이 이뤄졌는지 확인한뒤 올바를시 확인을 누르고, 아닐시 취소를 누르시오.' --width 900 --height 700 --column 자막 --column '번역된 가사' $list"
then
...
$(echo "$t2f" | sed -n $((ed+1)),\$p)" > "$st2"
	...
	echo '작업이 취소되었습니다.'
	read -n1 -rsp 'n키를 눌러 작업을 재시도하거나 아무키나 눌러 현재창을 닫습니다...' check
	echo
	case $check in
		[Nn])
			echo -e "프로세서를 재시작함.\n\n"
...
read -n1 -rsp '아무 키나 눌러 현재창을 닫습니다...'
echo
exit
